...
Langkah pertama yang dilakukan dalam penelitian adalah dengan memasukan \emph{dataset} untuk dilakukan \emph{pre-processing}. 
Dalam hal ini, gambar akan diberikan nama sesuai dengan jenis dan kelas ikan, penulis memberikan nama pada setiap gambar dengan format \emph{jenis-ikan\_kelas-ikan\_gambar ke} dan untuk jenis filenya \emph{jpg}, selengkapnya dapat dilihat pada gambar~\ref{fig:folder-ikan}.

\emph{pre-processing} dilakukan untuk memudahkan proses ektraksi sudut pada citra.~\emph{pre-processing} terbagi menjadi beberapa tahapan, yaitu penghilangan background, rotasi citra berdasarkan sudut PCA, dan smoothing citra dengan Gaussian blur.
...
Fungsi \emph{remove} digunakan untuk menghapus background dari gambar dengan menggunakan library \emph{rembg}. Contoh gambar dapat dilihat pada gambar~\ref{fig:dataset}.
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[keepaspectratio, width=14cm]{gambar/ikan_Nila_Induk_1_croped.jpg}}
	\caption{Contoh gambar dan jenis ikan yang digunakan}\label{fig:dataset}
\end{figure}

Parameter \emph{bgcolor} digunakan untuk menentukan warna background yang akan dihapus, dalam hal ini adalah warna putih dengan nilai RGB (255, 255, 255). Setelah fungsi ini dijalankan, gambar akan memiliki background transparan sehingga memudahkan proses ekstraksi sudut pada citra. Hasil dari proses remove dapat dilihat pada gambar~\ref{fig:rembg}.


\begin{figure}[H]
	\centering
	\fbox{\includegraphics[keepaspectratio, width=14cm]{gambar/IMG_20240121_135842 - Copy_croped.jpg}}
...
Setelah rotasi dilakukan dengan fungsi \emph{PCa\_Rotate\_image}, gambar akan memiliki bagian hitam hasil dari rotasi. Maka dari itu, bagian hitam akan di-isi dengan warna putih dengan menambahkan parameter \emph{borderValue} pada fungsi \emph{cv2.warpAffine}.
    ...
    \begin{figure}[H]
        \centering
        \fbox{\includegraphics[keepaspectratio, width=8cm]{gambar/ikan_Nila_Induk_1_output_croped.jpg}}
        \caption{Sebelum rotasi}\label{fig:before-rotasi}
    \end{figure}

    \begin{figure}[H]
        \centering
        \fbox{\includegraphics[keepaspectratio, width=8cm]{gambar/ikan_Nila_Induk_1_rotate2_croped.jpg}}
        \caption{Setelah rotasi}\label{fig:after-rotasi}
    \end{figure}
...
\emph{GaussianSmooth} bertujuan untuk menghaluskan citra dengan menggunakan filter Gaussian. Fungsi ini pertama-tama memeriksa apakah citra memiliki tiga saluran warna (RGB) atau hanya satu saluran (grayscale). Jika citra memiliki tiga saluran, maka akan dikonversi menjadi grayscale menggunakan fungsi \emph{cv2.cvtColor}. Setelah itu, fungsi \emph{cv2.GaussianBlur} digunakan untuk menerapkan filter Gaussian pada citra dengan kernel berukuran 3$\times$3 dan nilai sigmaX sebesar 1. 
Hasil dari proses ini kemudian dinormalisasi ke rentang [0, 255] menggunakan fungsi \emph{cv2.normalize} memastikan bahwa nilai piksel berada pada rentang yang valid untuk citra grayscale hasil nya dapat dilihat pada gambar~\ref{fig:gaussian}.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[keepaspectratio, width=8cm]{gambar/ikan_Nila_Induk_1_gaus_croped.jpg}}
    \caption{Contoh hasil gambar yang telah dilakukan smoothing Gaussian}\label{fig:gaussian}
\end{figure}

Setelah semua tahapan pre-processing telah selesai dilakukan, gambar akan disave kedalam folder terpisah dan akan digunakan untuk proses selanjutnya.
...
Tanpa proses smoothing, noise kecil pada citra dapat menghasilkan perubahan intensitas palsu yang berpotensi menyebabkan munculnya titik sudut yang tidak sesuai pada proses Harris-Corner Detection, hasil dapat dilihat pada Gambar~\ref{fig:csvgradien_x} untuk semua hasil csv dapat dilihat pada link github yang berada di lampiran.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{gambar/csv Gradient X_white.png}
    \caption{Hasil gradien pada sumbu x}\label{fig:csvgradien_x}
\end{figure}

\section{Auto Korelasi}
  Proses autokorelasi dilakukan dengan mengkalikan hasil dari gradien dengan gaussian blur. Proses ini bertujuan untuk mengukur seberapa besarnya perubahan intensitas,~berikut \emph{Source Code} yang digunakan untuk menghitung autokorelasi:
\begin{figure}[H]
    \begin{lstlisting}[language=Python, basicstyle=\tiny]
...
Nilai-nilai tersebut kemudian digunakan untuk membentuk matriks autokorelasi $M$ pada setiap piksel citra. Matriks ini digunakan sebagai dasar dalam menghitung nilai respon Harris pada tahap berikutnya.
Area yang memiliki perubahan intensitas tinggi pada dua arah sekaligus akan menghasilkan nilai autokorelasi yang besar. Kondisi ini umumnya terjadi pada bagian sudut objek ikan, seperti ujung kepala dan ekor. 
Sebaliknya, area tubuh ikan yang relatif datar akan menghasilkan perubahan intensitas yang lebih kecil sehingga nilai autokorelasi juga menjadi lebih rendah, salah satu hasil autokorelasi dapat dilihat pada Gambar~\ref{fig:csvautokorelasi} untuk semua hasil csv dapat dilihat pada link github yang berada di lampiran.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{gambar/csv Autokorelasi IXX_white.png}
    \caption{Hasil autokorelasi ixx}\label{fig:csvautokorelasi}
\end{figure}

\section{Harris Respon}
    Setelah matriks autokorelasi (\emph{second moment matrix}) diperoleh, tahap selanjutnya adalah menghitung nilai Harris response. 
...
Penelitian ini, menggunakan konstanta empiris $k$ sebesar 0.04. 
Konstanta tersebut digunakan untuk mengontrol sensitivitas deteksi sudut pada citra. 
...
\begin{figure}[H]
    \centering
    \includegraphics[keepaspectratio, width=14cm]{gambar/ikan_nila_induk_1_Harris.jpg}
...
Nilai threshold yang terlalu kecil dapat menyebabkan banyak titik sudut palsu (\emph{false corner}) ikut terdeteksi, sedangkan nilai threshold yang terlalu besar menyebabkan beberapa titik sudut penting pada objek ikan tidak terdeteksi.
...
kemudian, dibuat sebuah mask yang menunjukkan piksel-piksel yang memiliki nilai respon yang sama dengan hasil Thresholding. Hasil piksel-piksel tersebulah yang akan digunakan untuk menghitung panjang dan estimasi berat ikan pada tahap selanjutnya.  

\begin{figure}[H]
    \centering
    \includegraphics[keepaspectratio, width=14cm]{gambar/ikan_nila_induk_1_NMS.jpg}
...
Proses pembentukan bounding box dimulai dengan mencari:
\begin{itemize}
    \item nilai maximum dan minimum sumbu-$x$
    \item nilai maximum dan minimum sumbu-$y$
\end{itemize}
...
\begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=8cm]{gambar/ikan_nila_1 - Box_croped.jpg}
...
\subsection{Memproyeksi Perspektif}
...
Nilai referensi yang digunakan terdapat pada tabel~\ref{tab:Asli}.

Setelah dilakukan evaluasi akurasi terhadap hasil deteksi panjang, lebar, dan estimasi berat ikan, ditemukan bahwa beberapa hasil estimasi berat ikan ada yang kurang, bahkan ada yang melebihi dari nilai referensi-nya. Hasil deteksi dapat dilihat pada tabel~\ref{tab:Hasil Deteksi} yang terdapat pada lampiran. %Hal ini disebabkan oleh beberapa faktor, yaitu:
...
%         \hline
%         \endfirsthead
%         \multicolumn{5}{c}{{\tablename\ \thetable{} -- Lanjutan halaman sebelumnya}} \\
%         \hline
%         No. & Nama File & Panjang (cm) & Lebar (cm) & Berat (g) \\
%         \hline
%         \endhead
%         \hline \multicolumn{5}{|r|}{{Berlanjut dihalaman berikutnya}} \\ \hline
%         \endfoot
%         \hline
%         \endlastfoot
...
Dengan Menggunakan rumus MAPE, didapatkan hasil evaluasi rata-rata akurasi pada tabel berikut:
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|c|}
        ...
        \hline
    \end{tabular}
\caption{Rata-rata Akurasi setiap jenis ikan}
\end{table}
...
Berdasarkan hasil evaluasi akurasi pada tabel~\ref{tab:allEval} dan hasil deteksi pada tabel~\ref{tab:Hasil Deteksi} yang terdapat pada Lampiran, didapatkan kesimpulan bahwa sistem masih menghasilkan kesalahan dalam pengukuran pada beberapa citra ikan.
