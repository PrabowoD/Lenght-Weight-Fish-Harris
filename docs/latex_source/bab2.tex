...
Citra digital adalah citra yang terdiri dari sejumlah elemen terbatas, di mana setiap elemen memiliki lokasi dan nilai tertentu. 
Elemen-elemen ini disebut picture elements, image elements, pels, atau yang paling umum disebut sebagai piksel. (\cite{Gonzalez2018})

\section{Multi-scale representation}

  Bagian ini membahas representasi citra multi-resolusi berbasis filter Gaussian. 
Turunan Gaussian sering digunakan untuk mengidentifikasi fitur-fitur karakteristik. Pada \ref{subsec:scalespace} dijelaskan bagaimana menghitung stabilitas 
suatu gambar dan cara menormalisasi gambar adalah untuk memperoleh respons turunan yang tidak tergantung pada resolusi. 
Fitur lokal didefinisikan berdasarkan lokasi, skala, dan bentuk, yang akan mengalami transformasi saat dilihat dari sudut pandang berbeda. 
...
seperti dijelaskan pada \ref{subsex:SMM}, dengan komponen dari matriks Hessian digunakan untuk mendeteksi karakteristik skala struktur lokal dan mendeskripsikan bentuk struktur.
...
Tenemuan ini menyimpulkan bahwa konvolusi dengan kernel Gaussian merupakan solusi optimal untuk masalah konstruksi representasi multi-skala. Fungsi Gaussian dua dimensi didefinisikan sebagai berikut:
...
Sifat ini sangat berguna dalam praktik karena penghalusan sinyal dua dimensi dapat digantikan oleh dua penghalusan satu dimensi, satu untuk setiap dimensi. 
...
yaitu citra pada skala yang lebih kasar merupakan representasi yang disederhanakan dari citra pada skala yang lebih halus. 
...
Biasanya, ruang-skala yang seragam digunakan, tetapi representasi ruang-skala yang lebih umum dihitung menggunakan filter afine.
...
\subsection{Scale-space derivatives}
...
\section{Automatic scale selection}
...
\section{Scale invariant detector}
